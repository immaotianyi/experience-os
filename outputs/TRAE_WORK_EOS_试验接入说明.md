...
3. 在 TRAE 中仅调用 `eos_capture_collaboration` 保存一个明确同意的、高价值片段。每次调用都必须给出：项目 ID、来源工具、来源引用、事件摘要和 `consented: true`。

4. 回到 EOS，检查 `eos_project_timeline` 或项目页，确认该片段以来源 -> Evidence -> WorkCheckpoint 的可追溯链路出现。

5. 人类选择工作节点后，生成 Experience Receipt Draft；只在阅读草案、确认适用边界与不确定性后，才确认成正式 Receipt。

6. 在真实任务产生结果后，记录人工决策与 Outcome。只有证据、决策和成功结果完整闭环后，EOS 才能升级 ExperienceAsset，并在后续项目提出有边界的建议。

## TRAE 同事的权限

允许：读取已获授权的 EOS 状态；保存人类明确同意的片段；读取项目时间线、升级资格和已验证经验。
